 ...
 * This is the final compatibility guard for the Engine runtime.
 */
(() => {
  'use strict';
  ...
  e.saveLiveCharacter=c=>{const out=e.normalizeCharacter?e.normalizeCharacter(c):c;cleanStaleArmorModifiers(out);const saved=e.saveCharacter(out);cleanLegacyStorage();return saved;};
  e.normalizeCharacter=c=>{
    ...
    c.resources.deathSaves.successes=clamp(Math.floor(n(c.resources.deathSaves.successes)),0,3);c.resources.deathSaves.failures=clamp(Math.floor(n(c.resources.deathSaves.failures)),0,3);c.currency=c.currency||{copper:0,silver:0,gold:0,platinum:0};cleanStaleArmorModifiers(c);return c;
  };
