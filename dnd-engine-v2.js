...
(() => {
  ...
  const pb = level => 2 + Math.floor(Math.max(1,Number(level)||1)-1)/4 | 0;
  const deep = (base, extra) => {
    if (Array.isArray(base)) return Array.isArray(extra) ? clone(extra) : clone(base);
    if (base && typeof base==='object') { const o={}; const e=extra&&typeof extra==='object'&&!Array.isArray(extra)?extra:{}; Object.keys(base).forEach(k=>o[k]=deep(base[k],e[k])); Object.keys(e).forEach(k=>{if(!(k in o))o[k]=e[k]}); return o; }
  ...
  function skill(c,id){const a=D.SKILLS[id];if(!a)return 0;const p=c.proficiencies?.skills?.[id]||{};let v=mod(effectiveAbility(c,a));if(p.proficiency)v+=profBonus(c);if(p.expertise)v+=profBonus(c);return apply(v,modifiers(c,`skill:${id}`));}
  ...
  function features(c=state){const out=[];classLevels(c).forEach(cl=>{(D.CLASS_FEATURES[cl.classId]||[]).filter(f=>f.level<=(Number(cl.level)||0)).forEach(f=>out.push({...f,classId:cl.classId}))});return out;}
  ...
  function ac(c=state){const ar=armor(c),sh=shield(c),dex=mod(effectiveAbility(c,'dexterity'));let v=10+dex;if(ar){const m=ar.mechanics||{};const base=Number(m.armorClass)||10;const cat=m.category||'light';if(cat==='heavy')v=base;else if(cat==='medium')v=base+Math.min(dex,Number(m.dexterity?.maximum??2));else v=base+dex;}if(sh)v+=Number(sh.mechanics?.armorBonus)||2; if(hasFeature(c,'unarmored-defense-barbarian')&&!ar)v=10+mod(effectiveAbility(c,'dexterity'))+mod(effectiveAbility(c,'constitution'));if(hasFeature(c,'unarmored-defense-monk')&&!ar)v=10+mod(effectiveAbility(c,'dexterity'))+mod(effectiveAbility(c,'wisdom'));return apply(v,modifiers(c,'armorClass'));}
  ...
  function spellAbility(c=state){for(const cl of classLevels(c)){const d=classData(cl.classId);if(d?.spell&&Number(cl.level)>0)return d.spell;}return null;}
  function spellAttack(c=state){const a=spellAbility(c);return a?apply(mod(effectiveAbility(c,a))+profBonus(c),modifiers(c,'spellAttackBonus')):null;}
  function spellDC(c=state){const a=spellAbility(c);return a?apply(8+mod(effectiveAbility(c,a))+profBonus(c),modifiers(c,'spellSaveDC')):null;}
  function spellcasterLevel(c=state){return classLevels(c).reduce((n,cl)=>{const d=classData(cl.classId),l=Number(cl.level)||0;if(!d?.spell)return n;if(cl.classId==='warlock')return n; if(cl.classId==='artificer')return n+Math.ceil(l/2);return n+l},0)}
  function slots(c=state){const l=Math.min(20,spellcasterLevel(c));const row=D.SPELL_SLOTS[l]||[];return Object.fromEntries(row.map((n,i)=>[i+1,n]));}
  ...
  function hpMax(c=state){let total=0;let first=true;classLevels(c).forEach(cl=>{const die=classData(cl.classId)?.hitDie||8;const l=Math.max(0,Number(cl.level)||0);if(!l)return;if(first){total+=die;total+=(l-1)*(Math.floor(die/2)+1);first=false}else total+=l*(Math.floor(die/2)+1);});total+=mod(effectiveAbility(c,'constitution'))*totalLevel(c);return Math.max(1,apply(total,modifiers(c,'hitPointMaximum')));}
  ...
  function attack(c,item){if(!item||item.mechanics?.type!=='weapon')return null;const a=weaponAbility(c,item),atk=item.mechanics.attack||{};return {ability:a,bonus:apply(abilityMod(c,a)+(atk.proficient!==false?profBonus(c):0)+(Number(atk.bonus)||0),modifiers(c,'weaponAttackBonus')),damage:(item.mechanics.damage||[]).map(d=>({dice:d.dice,type:d.type,modifier:(Number(d.modifier)||0)+abilityMod(c,d.ability||a)}))};}
  function summary(c=state){const hp=hpMax(c);const sourceSummary=Object.values(D.SOURCES).map(s=>({...s,enabled:s.source==='phb2014'||c.rules.sources[s.id]!==false}));return {level:totalLevel(c),proficiencyBonus:profBonus(c),abilities:Object.fromEntries(D.ABILITIES.map(a=>[a,{score:effectiveAbility(c,a),modifier:mod(effectiveAbility(c,a))}])),ac:ac(c),speed:speed(c),initiative:initiative(c),carryingCapacity:carrying(c),carriedWeight:weight(c),hpMaximum:hp,spellAbility:spellAbility(c),spellAttackBonus:spellAttack(c),spellSaveDC:spellDC(c),spellSlots:slots(c),hitDice:hitDice(c),subclasses:subclasses(c),features:features(c),sources:sourceSummary};}
  window.DnDEngineV2={get state(){return state},load,save,set,create,summary,abilityScore,abilityMod,effectiveAbility,skill,saveThrow,profBonus,totalLevel,classLevels,classLevel,subclasses,features,hasFeature,ac,speed,initiative,carrying,weight,spellAbility,spellAttack,spellDC,spellcasterLevel,slots,hpMax,hitDice,attack,modifiers,D};
})();