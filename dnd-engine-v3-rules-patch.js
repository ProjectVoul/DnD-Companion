...
(() => {
  ...
  const spellAbility=(c,sourceClass)=>{
    if(sourceClass){const data=D.CLASSES?.[sourceClass];if(data?.spell)return data.spell;}
  ...
  const collectEffects=c=>{
    ...
    (c.items||[]).forEach(i=>{if(i.equipment?.equipped&&(!requiresAttunement(i)||i.equipment?.attuned)){(i.effects||[]).forEach(e=>add(e,i.name||'Item','item'));(i.modifiers||[]).forEach(m=>add({...m,effectType:'modifier'},i.name||'Item','item'));}});
  ...
  const saveThrow=(c,ability)=>{
    let value=originalSaveThrow(c,ability);
    const unconscious=(c.conditions||[]).some(x=>(typeof x==='string'?x:x.id)==='unconscious');
    const paladin=(c.classes||[]).some(cl=>cl.classId==='paladin'&&num(cl.level)>=6);
    if(paladin&&!unconscious)value+=Math.max(1,E.abilityMod(c,'charisma'));
    return value;
  };
  const originalHasImmunity=E.hasImmunity;
  const hasImmunity=(c,type)=>{
    if(type==='frightened'&&(c.classes||[]).some(cl=>cl.classId==='paladin'&&num(cl.level)>=10)&&!(c.conditions||[]).some(x=>(typeof x==='string'?x:x.id)==='unconscious'))return true;
    return originalHasImmunity(c,type);
  };
  ...
  Object.assign(E,{spellcastingProfiles:profiles,spellcasterLevel,spellAbility,spellAttack,spellDC,spellSlots,slots:spellSlots,extraAttacks,collectEffects,modifiers,attunedItems,magicItemCount,attunementCapacity,canAttuneItem,canUseItem,saveThrow,hasImmunity,features:featureSnapshot,summary});
