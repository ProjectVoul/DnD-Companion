...
for(const spell of SPELLS){
 ...
 if(needsMechanics&&!hasMechanics)missing++;
}
assert.equal(missing,0,`Spell technical metadata audit: ${missing} spell(s) still need structured mechanics. Add save/attack/damage/area metadata from the source before declaring the catalog complete.`);
