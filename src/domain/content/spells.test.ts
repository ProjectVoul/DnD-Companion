...
for(const spell of [...SPELLS,...PALADIN_SPELLS,...PALADIN_SUPPLEMENTAL_SPELLS]){assert.ok(spell.source,'Every catalogued spell must declare a source');assert.ok(CONTENT_SOURCES.some(source=>source.id===spell.source),'Every spell source must be known');assert.ok(spell.id,'Every spell must have an id');}
const all=[...SPELLS,...PALADIN_SPELLS,...PALADIN_SUPPLEMENTAL_SPELLS];
const ids=new Map<string,number>();
for(const spell of all)ids.set(spell.id,(ids.get(spell.id)??0)+1);
const duplicates=[...ids.entries()].filter(([,count])=>count>1);
assert.deepEqual(duplicates,[],'Duplicate spell IDs are forbidden; overlapping source catalogs must deduplicate by id');
...
const catalog=all.filter((spell,index,array)=>array.findIndex(other=>other.id===spell.id)===index);
