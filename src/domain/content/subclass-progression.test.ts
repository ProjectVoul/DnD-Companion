...
for(const [classId,subclasses] of Object.entries(ALL_SUBCLASSES)){
 for(const subclass of subclasses as {id:string;name:string}[]){
  ...
  for(const feature of resolved){
   assert.equal(feature.subclassId,subclass.id,`Cross-subclass feature leaked: ${classId}/${subclass.id}/${feature.id}`);
...
// Concrete content must never be silently replaced by generic placeholder text.
for(const feature of subclassFeaturesFor('fighter','fighter.champion',20)){
 assert.notEqual(feature.name,'Champion Feature');
}
