...
for(const access of SUBCLASS_SPELL_ACCESS){
 ...
 for(const id of access.alwaysPrepared){const spell=ALL_SPELLS.find(s=>s.id===id);assert.ok(spell,`Missing canonical spell ${id} for ${access.classId}/${access.subclassId}`);assert.ok(spell.classes.includes(access.classId),`Subclass spell ${id} is not on ${access.classId} class list`);}
}
...
console.log('Subclass spell-access invariants passed');
