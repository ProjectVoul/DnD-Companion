...
import {applyMulticlassProficiencies,multiclassChoiceValid} from './rules/multiclass';
...
assert.equal(allowed.weaponProficiencies?.includes('simple'),true);
assert.equal(allowed.weaponProficiencies?.includes('shortsword'),true);

const rogueChoice={...base,skillStates:{...base.skillStates,stealth:{proficient:false,expertise:false}}};
assert.equal(multiclassChoiceValid('rogue',{skills:['stealth'],tools:['thieves-tools']}),true);
assert.equal(multiclassChoiceValid('rogue',{skills:['stealth']}),false);
const rogueGranted=applyMulticlassProficiencies(rogueChoice,'rogue',{skills:['stealth'],tools:['thieves-tools']});
assert.equal(rogueGranted.skillStates.stealth.proficient,true);
assert.equal(rogueGranted.toolProficiencies?.includes('thieves-tools'),true);
assert.equal(rogueGranted.armorProficiencies?.includes('light'),true);
const artificerGranted=applyMulticlassProficiencies(base,'artificer');
assert.deepEqual(artificerGranted.toolProficiencies,['thieves-tools','tinkers-tools']);
assert.equal(artificerGranted.armorProficiencies?.includes('medium'),true);
console.log('multiclass prerequisite and proficiency tests passed');
