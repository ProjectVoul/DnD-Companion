...
export function weaponDamageBonus(c:Character,item:Item,options:AttackOptions={}){const w=item.weapon;if(!w)return 0;let bonus=derivedAbilityMod(c,w.attackAbility)+w.damageBonus+effectBonus(c,'damage');const equippedWeapons=c.items.filter(i=>i.equipped&&i.kind==='weapon');if(c.fightingStyles?.includes('dueling')&&equippedWeapons.length===1&&!isRanged(item)&&!property(item,'two-handed'))bonus+=2;if(options.powerAttack&&hasFeat(c,isRanged(item)?'sharpshooter':'great-weapon-master')&&(isRanged(item)||property(item,'heavy')))bonus+=10;return bonus}
