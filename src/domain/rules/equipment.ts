...
function style(c:Character){return c.fightingStyles?.[0]?.toLowerCase()??'';}
function hasFeat(c:Character,id:string){return (c.feats??[]).some(name=>(featDefinition(name)?.id??name)===id)}
export function weaponAttackBonus(c:Character,item:Item,options:AttackOptions={}){const w=item.weapon;if(!w)return 0;let bonus=derivedAbilityMod(c,w.attackAbility)+(w.proficient?c.proficiencyBonus:0)+w.attackBonus+effectBonus(c,'attack');if(style(c)==='archery'&&isRanged(item))bonus+=2;if(options.powerAttack&&hasFeat(c,isRanged(item)?'sharpshooter':'great-weapon-master')&&(isRanged(item)||property(item,'heavy')))bonus-=5;return bonus}
export function weaponDamageParts(item:Item){return item.weapon?.damage??[]}
export function weaponDamageBonus(c:Character,item:Item,options:AttackOptions={}){const w=item.weapon;if(!w)return 0;let bonus=derivedAbilityMod(c,w.attackAbility)+w.damageBonus+effectBonus(c,'damage');const equippedWeapons=c.items.filter(i=>i.equipped&&i.kind==='weapon');if(style(c)==='dueling'&&equippedWeapons.length===1&&!isRanged(item)&&!property(item,'two-handed'))bonus+=2;if(style(c)==='thrown-weapon-fighting'&&property(item,'thrown')&&isRanged(item))bonus+=2;if(options.powerAttack&&hasFeat(c,isRanged(item)?'sharpshooter':'great-weapon-master')&&(isRanged(item)||property(item,'heavy')))bonus+=10;return bonus}
