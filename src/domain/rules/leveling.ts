...
export function levelUp(c:Character,classId:string,mode:HPMode='average',abilityIncrease?:{ability?:Ability;amount?:number;secondAbility?:Ability}):Character{
 const target=c.classes.find(x=>x.id===classId);if(!target||target.level>=20||c.level>=20)return c;
 const next=structuredClone(c);const cls=CLASSES.find(x=>x.id===classId);if(!cls)return c;
 ...
 if(cls.spellcasting!=='none'){
  const slots=multiclassSpellSlots(next);const state=next.spellcasting[classId]??{known:[],prepared:[],alwaysPrepared:[],spellbook:[],slots:{}};next.spellcasting[classId]={...state,slots};
 }
 const gained=(CLASS_PROGRESSION[classId]??[]).filter(f=>f.level===target.level);const existing=new Set(next.features.map(f=>f.id));gained.forEach(f=>{if(!existing.has(f.id))next.features.push(f)});
 return next;
}
