...
import {applyMulticlassProficiencies,multiclassChoiceValid} from './multiclass';
...
export function canMulticlassInto(c:Character,classId:string){const current=c.classes.find(x=>x.id===classId);if(current)return true;if(!meetsRequirements(c,classId))return false;return c.classes.every(cl=>meetsRequirements(c,cl.id));}
export function levelUp(c:Character,classId:string,mode:HPMode='average',abilityIncrease?:{ability?:Ability;amount?:number;secondAbility?:Ability},featSelection?:{id:string;choice?:FeatChoice},multiclassChoice?:MulticlassChoice):Character{
 ...
 const addingNewClass=!current;if(addingNewClass&&!canMulticlassInto(c,classId))return c;if(addingNewClass&&!multiclassChoiceValid(classId,multiclassChoice))return c;
 const nextClassLevel=current?current.level+1:1;const asi=hasASI(classId,nextClassLevel);if(abilityIncrease&&featSelection)return c;
 ...
 const next=structuredClone(c);let target=next.classes.find(x=>x.id===classId);const cls=CLASSES.find(x=>x.id===classId);if(!cls)return c;if(featSelection&&!featCanBeSelected(next,featSelection.id,featSelection.choice))return c;
 const oldMaxHP=derivedMaxHP(c);const oldConMod=derivedAbilityMod(c,'con');const priorClassLevel=current?.level??0;if(!target){target={id:classId,name:cls.name,level:0,spellcastingAbility:cls.spellcastingAbility};next.classes.push(target);}
 ...
 const pool=next.hitDice.pools.find(p=>p.die===cls.hitDie);if(pool){pool.max+=1;pool.current+=1;}else next.hitDice.pools.push({die:cls.hitDie,max:1,current:1});if(addingNewClass)Object.assign(next,applyMulticlassProficiencies(next,classId,multiclassChoice));
