...
export function levelUp(c:Character,classId:string,mode:HPMode='average',abilityIncrease?:{ability?:Ability;amount?:number;secondAbility?:Ability},featSelection?:{id:string;choice?:FeatChoice}):Character{
 const current=c.classes.find(x=>x.id===classId);if(!current&&c.level>=20)return c;
 if(current&&current.level>=20||c.level>=20)return c;
 ...
 const next=structuredClone(c);let target=next.classes.find(x=>x.id===classId);const cls=CLASSES.find(x=>x.id===classId);if(!cls)return c;
 ...
 if(!target){target={id:classId,name:cls.name,level:0,spellcastingAbility:cls.spellcastingAbility};next.classes.push(target);}
