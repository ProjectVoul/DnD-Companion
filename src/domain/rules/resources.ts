...
export function classResources(c:Character):Resource[]{
 ...
 for(const cl of c.classes){switch(cl.id){case'barbarian':if(cl.level>=1)push('rage','Rage',cl.level>=20?0:cl.level>=17?6:cl.level>=12?5:cl.level>=6?4:cl.level>=3?3:2,'long',cl.level>=20,cl.id);break;case'bard':if(cl.level>=1)push('bardic-inspiration','Bardic Inspiration',Math.max(1,Math.floor((c.abilityScores.cha-10)/2)),cl.level>=5?'short':'long',false,cl.id);break;case'cleric':if(cl.level>=2)channelDivinity=Math.max(channelDivinity,cl.level>=18?3:cl.level>=6?2:1);break;case'druid':if(cl.level>=2)push('wild-shape','Wild Shape',cl.level>=20?0:2,'short',cl.level>=20,cl.id);break;case'monk':if(cl.level>=2)push('ki','Ki',cl.level,'short',false,cl.id);break;case'paladin':if(cl.level>=1)push('lay-on-hands','Lay on Hands',cl.level*5,'long',false,cl.id);if(cl.level>=3)channelDivinity=Math.max(channelDivinity,1);break;case'sorcerer':if(cl.level>=2)push('sorcery-points','Sorcery Points',cl.level,'long',false,cl.id);break;case'fighter':if(cl.level>=1)push('second-wind','Second Wind',cl.level>=17?2:1,'short',false,cl.id);if(cl.level>=2)push('action-surge','Action Surge',cl.level>=17?2:1,'short',false,cl.id);break;default:break;}}
