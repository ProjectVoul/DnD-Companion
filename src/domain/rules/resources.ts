...
export function classResources(c:Character):Resource[]{
 ...
 case'paladin':if(cl.level>=1)push('lay-on-hands','Lay on Hands',cl.level*5,'long',false,cl.id,'Healing pool equal to five times paladin level; replenished on a long rest.');if(cl.level>=3)channelDivinity=Math.max(channelDivinity,1);if(cl.level>=1)push('divine-sense','Divine Sense',Math.max(1,1+abilityMod(c.abilityScores.cha)),'long',false,cl.id,'Uses equal to 1 + Charisma modifier (minimum one); replenished on a long rest.');if(cl.level>=14)push('cleansing-touch','Cleansing Touch',Math.max(1,abilityMod(c.abilityScores.cha)),'long',false,cl.id,'Uses equal to Charisma modifier (minimum one); replenished on a long rest.');break;
 case'ranger':break;
