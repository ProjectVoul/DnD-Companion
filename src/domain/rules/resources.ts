...
export function classResources(c:Character):Resource[]{
 ...
 case'bard':if(cl.level>=1)push('bardic-inspiration','Bardic Inspiration',Math.max(1,Math.floor((c.abilityScores.cha-10)/2)),cl.level>=5?'short':'long',false,cl.id,'Uses equal to Charisma modifier; recharge changes to short rest at 5th level.');break;
 ...
 case'paladin':if(cl.level>=1)push('lay-on-hands','Lay on Hands',cl.level*5,'long',false,cl.id,'Healing pool equal to five times paladin level; replenished on a long rest.');if(cl.level>=3)channelDivinity=Math.max(channelDivinity,1);if(cl.level>=1)push('divine-sense','Divine Sense',1+(c.abilityScores.cha>=10?Math.floor((c.abilityScores.cha-10)/2):Math.floor((c.abilityScores.cha-10)/2)),'long',false,cl.id,'Uses equal to 1 + Charisma modifier; replenished on a long rest.');if(cl.level>=14)push('cleansing-touch','Cleansing Touch',Math.max(0,Math.floor((c.abilityScores.cha-10)/2)),'long',false,cl.id,'Uses equal to Charisma modifier; replenished on a long rest.');break;
 case'ranger':break;
 case'sorcerer':if(cl.level>=2)push('sorcery-points','Sorcery Points',cl.level,'long',false,cl.id,'Sorcery points equal to sorcerer level; recovered on a long rest.');if(cl.level>=20)push('sorcerous-restoration','Sorcerous Restoration',4,'long',false,cl.id,'Regains 4 expended sorcery points on a short rest once between long rests.');break;
 ...
 case'artificer':if(cl.level>=7)push('flash-of-genius','Flash of Genius',Math.max(0,Math.floor((c.abilityScores.int-10)/2)),'long',false,cl.id,'Uses equal to Intelligence modifier; replenished on a long rest.');break;
 ...
 default:break;}}
 if(channelDivinity)out.push({id:'shared:channel-divinity',name:'Channel Divinity',current:channelDivinity,max:channelDivinity,recharge:'short',sourceId:'shared:channel-divinity',description:'Channel Divinity uses are recovered on a short or long rest.'});
