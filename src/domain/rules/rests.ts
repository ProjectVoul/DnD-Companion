...
export function applyRest(c:Character,type:RestType):Character{
 ...
 next.currentHP=next.maxHP;next.tempHP=0;next.deathSaves={successes:0,failures:0};
...
export function spendHitDie(c:Character):Character{
 ...
 next.hitDice.current-=1;next.currentHP=Math.min(next.maxHP,next.currentHP+healing);if(next.currentHP>0)next.deathSaves={successes:0,failures:0};return next;
}
