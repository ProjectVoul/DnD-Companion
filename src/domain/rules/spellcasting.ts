...
export function beginConcentration(c:Character,classId:string,spell:Spell):Character{if(!spell.concentration)return c;return {...c,concentration:{spellId:spell.id,classId}}}
export function endConcentration(c:Character):Character{if(!c.concentration)return c;const {concentration:_,...rest}=c;return rest}
export function concentrationSaveDC(damage:number){return Math.max(10,Math.floor(damage/2))}
