...
export interface SpellMechanics{attack?:SpellAttackType;save?:Ability;damage?:DamagePart[];halfDamageOnSave?:boolean;area?:string;targets?:string;conditions?:string[];special?:string;}
export interface Effect{ id:string;name:string;target:'ac'|'savingThrows'|'initiative'|'speed'|'darkvision'|'skill'|'skillProficiency'|'skillExpertise'|'attack'|'damage'|'maxHP'|'ability'|'passivePerception'|'passiveInvestigation';value:number;ability?:Ability;skill?:Skill;damageType?:DamageType;condition?:string;passive?:boolean;sourceId?:string;description?:string; }
...
export interface Spell{id:string;name:string;level:number;school:string;classes:string[];ritual?:boolean;concentration?:boolean;source?:string;alwaysPrepared?:boolean;castingTime?:string;range?:string;components?:string[];duration?:string;description?:string;higherLevels?:string;mechanics?:SpellMechanics;}
