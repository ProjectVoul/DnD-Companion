...
export function Builder({onCreate}:{onCreate:(c:Character)=>void}){
 ...
 const setScore=(a:Ability,value:number)=>setScores(x=>{const other=Object.entries(x).find(([key,v])=>key!==a&&v===value)?.[0];if(other)return {...x,[a]:value,[other]:x[a]};return {...x,[a]:value}});
 const toggleSkill=(s:Skill)=>{if(!skillOptions.includes(s))return;setC(x=>{const on=x.skillStates[s].proficient;if(on)return {...x,skillStates:{...x.skillStates,[s]:{proficient:false,expertise:false}}};const count=Object.values(x.skillStates).filter(v=>v.proficient).length;if(count>=CLASS_SKILL_COUNT[cls])return x;return {...x,skillStates:{...x.skillStates,[s]:{proficient:true,expertise:false}}};});};
 const finish=()=>{const conMod=mod(scores.con);const hp=Math.max(1,(c.classes[0]?.level===1?CLASSES.find(x=>x.id===cls)?.hitDie??8:c.maxHP)+conMod);onCreate({...c,abilityScores:scores,proficiencyBonus:2,maxHP:hp,currentHP:hp})};
 return <section className="card builder"><div className="row"><div><h2>Create Character</h2><p className="muted">5e 2014 character builder. Every choice is explicit; optional sources remain opt-in.</p></div><b>Step {step} / 4</b></div>
 ...
 {step===3&&<><h3>Ability Scores</h3><p className="muted">Assign the standard array exactly once: 15, 14, 13, 12, 10, 8.</p><div className="ability-builder">{ABILITIES.map(a=><label key={a}>{a.toUpperCase()}<select value={scores[a]} onChange={e=>setScore(a,Number(e.target.value))}>{STANDARD_ARRAY.map(v=><option key={v} value={v} disabled={usedScores.has(v)&&scores[a]!==v}>{v}</option>)}</select><small>modifier {mod(scores[a])>=0?'+':''}{mod(scores[a])}</small></label>)}</div><button onClick={()=>setStep(4)}>Continue</button></>}
