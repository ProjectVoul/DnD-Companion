...
function ItemModal({item,update,close,remove}:{item:Item;update:(fn:(x:Character)=>Character)=>void;close:()=>void;remove:()=>void}){
  ...
  return <div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="modal item-modal">
    ...
    <div className="form-grid"><label>Name<input value={item.name} onChange={e=>patch({name:e.target.value})}/></label><label>Type<select value={item.kind} onChange={e=>changeKind(e.target.value as ItemKind)}>{kinds.map(k=><option key={k}>{k}</option>)}</select></label><label>Quantity<input type="number" min="1" value={item.quantity} onChange={e=>patch({quantity:Math.max(1,Number(e.target.value))})}/></label><label>Weight (lb)<input type="number" min="0" step="0.1" value={item.weight??0} onChange={e=>patch({weight:Math.max(0,Number(e.target.value))})}/></label></div>
    <label>Description<textarea value={item.description??''} onChange={e=>patch({description:e.target.value})}/></label><label>Effects<textarea value={effects.join('\n')} placeholder="One descriptive effect per line" onChange={e=>patch({effects:e.target.value.split('\n').map(x=>x.trim()).filter(Boolean)})}/></label>
