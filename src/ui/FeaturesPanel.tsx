...
export function FeaturesPanel({c,onResource}:{c:Character;onResource?:(resourceId:string)=>void}){
 ...
 const features=[...c.features,...c.classes.flatMap(cl=>(CLASS_PROGRESSION[cl.id]??[]).filter(f=>f.level<=cl.level)),...optional].map(f=>({...f,description:f.description??FEATURE_DESCRIPTIONS[f.id]??FEATURE_DESCRIPTIONS[`${f.id.split(':')[0]}:${f.id.split(':')[1]}`]}));
 ...
 {c.feats?.length?<><h3>Feats</h3>{c.feats.map((feat,i)=><button className="feature feature-button" key={`${feat}-${i}`} onClick={()=>setSelected({id:`feat:${i}`,name:feat,source:'feat',level:0,description:'Feat details will come from the canonical feat content catalog.'})}><div className="row"><b>{feat}</b><small>Feat</small></div></button>)}</>:null}
