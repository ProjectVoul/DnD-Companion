...
export function FeaturesPanel({c}:{c:Character}){
 ...
 {selected&&<FeatureModal feature={selected} close={()=>setSelected(null)}/>}</section>
}
function FeatureModal({feature,close}:{feature:Feature;close:()=>void}){return <div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="modal"><div className="row"><div><span className="eyebrow">{feature.source} · Level {feature.level}</span><h2>{feature.name}</h2></div><button onClick={close}>×</button></div><p className="feature-description">{feature.description??'The rule text for this feature is not yet populated in the content catalog.'}</p>{feature.activation&&<p><b>Activation:</b> {feature.activation}</p>}{feature.effects?.length?<><h3>Mechanical effects</h3>{feature.effects.map(e=><div className="effect-line" key={e.id}><b>{e.target}</b><span>{e.value>=0?'+':''}{e.value}{e.ability?` ${e.ability.toUpperCase()}`:''}{e.skill?` · ${e.skill}`:''}</span></div>)}</>:null}{feature.resourceId&&<p><b>Resource:</b> {feature.resourceId}</p>}</div></div>}
